...
import CompanyDetailDesc from "./CompanyDetailDesc";
import SkeletonCompanyDetail from "./SkeletonCompanyDetail";

interface IProps {
  id: string;
}

const CompanyDetailContent = async ({ id }: IProps) => {
  try {
    const res = await companyApiRequest.callFetchCompanyById(id);
